...
*decomposition of order* $p$ of $X$. Decompositions of various kinds play a key role in 
multivariate data analysis, and in theoretical and numerical linear algebra (@stewart_98). 
...
::: {#lem-construct}
The $n\times p$ orthonormal $Y$ maximizes $\text{tr}\ Y'Z$, with 
\begin{equation}
...
It is important to note that in MDFA we cannot use $Y=KL'$, because then $Y'Y=LL'$, which is of rank $r\leq m$ and thus not equal to the identity if $r<p$. Also note that formula \eqref{eq-yoptt} is identical to formula \eqref{eq-yperf} in the fundamental theorem. This is not surprising, since both compute the optimal $Y$ for given $T$. The fundamental theorem merely adds that the optimal $Y$ makes loss equal to zero if $C=TT'$.
...
The CFA equations \eqref{eq-cfa1}-\eqref{eq-cfa6} imply that $0\lesssim AA'\lesssim C$ and $0\lesssim D^2\lesssim C$. The second one in turn implies that $0\lesssim D^2\lesssim\text{diag}(C)$. In most CFA techniques it is possible that the estimated uniquenesses do not satisfy these inequality constraints
...
so not all improper solutions are excluded. If $X'U$ is diagonal at the solution then indeed 
$D^2=X'UU'X\lesssim C$. 

The repository has a file mdfaAlgorithmA.R, with the function mdfaAlgorithmA() implementing
Algorithm A. There is an argument proj which is a function that takes care of the least squares projections on $\mathcal{T}$. By default proj is mdfaCFAProject(), which replaces the last $m$ columns of $T$ by its diagonal, but changing the default projection routine allows us to incorporate any set of constraints. 
...
adding $K_\perp SL_\perp'$ in each iteration we would get the same sequence of loss function values and the same
...
advantages in secondary analysis. And factor analysis method publications tend to have
secondary analyses, using the mountain of correlation matrices 
that have been published since 1900.

::: {#thm-als}
Suppose $T$ has rank $m$. Then the uodate is
\begin{equation}
...
$$
\sigma_\star(T)\leq\text{tr}\ C+\text{tr}\ T'T-2\ \text{tr}\ T'\overline G,
$$
with equality if $T=\overline T$. Since $\|XT\|_\tau=\max_{Y'Y=I}\text{tr}\ Y'XT$ it follows from the theorem about subgradients on linear transformations (@rockafellar_70, theorem 23.9) and the theorem about subgradients of supremum functions (@mordukhovich_nam_22, section 4.4) that
$$
...
this generalizes Algorithm B.

In Algorithm A we compute the determinate part of $Y$, and optionally after convergence a proper orthonormal $Y$. In Algorithm B there is no $Y$ at all, and if there is no $X$ we c annot compoute factor scores,  In both cases finding appropiate (indeterminate) factor scores $Y$ requires additional work, not unlike what must be done in the MINRES or ML techniques for CFA. One option is to simply compute the optimal $T$ and then use one of the various classical methods to compute factor scores (@mcdonald_burr_67).
...
on the set of orthonormal $(n-m)\times q$ matrices. We assume that $U_\perp'XA$ has rank $q$, so that the projection is unique. 
...
We do not want to compute with bases for null-spaces such as $U_\perp$, but we can use $U_\perp^{\ } U_\perp'=I-UU'$. Also define $\tilde D:=U'X$. Then
\begin{equation}
F\leftarrow(X-UD)A
(A'(C-\tilde D'\tilde D)A)^{-1/2}.\label{eq-fupdatesimp}
\end{equation}
In the same way the optimum $U$ for given $(F,A,D)$ is
\begin{equation}
U\leftarrow (X-F\tilde A)D
(D(C-\tilde A\tilde A')D)^{-1/2},\label{eq-uupdate}
\end{equation}
where $\tilde A=X'F$.
...
\begin{equation}
A\leftarrow\Pi_{\mathcal{A}}(X'F)=\Pi_{\mathcal{A}}
(C-\tilde D\tilde D')A
(A'(C-\tilde D\tilde D')A)^{-1/2})
.\label{eq-aupdate}
\end{equation}
Then \eqref{eq-aupdate} becomes
\begin{subequations}
\begin{equation}
A\leftarrow \Pi_{\mathcal{A}}\left\{(C-\tilde D\tilde D')A[A'(C-\tilde D\tilde D')A]^{-1/2}\right\}.\label{eq-aupdate2}
\end{equation}
In the same way, using  $F_\perp^{\ } F_\perp'=I-FF'$ and $\tilde A:=F'X$, gives the update
\begin{equation}
D\leftarrow\Pi_{\mathcal{D}}(X'U)=\Pi_{\mathcal{D}}\left\{(C-\tilde A\tilde A')D
