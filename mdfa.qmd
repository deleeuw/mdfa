...
There are several factor analytic decomposition techniques that use least squares loss functions. The oldest and most obvious one is
\begin{equation}
...
\end{equation}
An alternating least squares technique to minimize \eqref{eq-minrescfa} was first proposed by @thomson_34. It alternates minimizing the loss function over $D$ for the current $A$ and minimizing over $A$ for the current $D$. Of course this only finds an approximate solution to the reduced system, and it leaves open the question on how to compute the factor scores $Y$. Alternative algorithms for minimizing \eqref{eq-minrescfa}
...
\begin{equation}
\sigma(T,F)=\text{tr}\ (X-FA')D^{-2}(X-FA')',\label{eq-young-whittle}
\end{equation}
...
It is tempting to use block relaxation here, alternating finding the optimum
...
\end{equation}
But \eqref{eq-lawley41} is the negative maximum likelihood loss function for the fixed factor model from @lawley_41, and we know this loss function is unbounded below, does not have a minimum, and converges to a solution which has $p$ of the uniquenesses in $D^2$ equal to zero (@anderson_rubin_56).
...
version of MDFA and a version for a "wide" data matrix $X$ (@trendafilov_unkel_11)
There is a nice review of these contributions in @unkel_trendafilov_10, with an update in
...
K. Adachi contributed some useful results to what they initially called *Data-Fitting Factor Analysis*. @adachi_12, @adachi_trendafilov_18

@yamashita_25, @terada_25
...
the optimum $Y$ for given $T$ and the optimum $T$ for given $Y$.
...
$X'Y$ on the convex set of matrices $\mathcal{T}$. 
...
equal to each other and/or others to be equal to given constants finding $T$ is still a simple linear least squares problem. If there are inequality constraints we need some form of quadratic programming.
...
*trace norm* $\|Z\|_\star$ of a matrix $Z$ is the sum of its singular values. The nuclear norm is also equal to the sum of the square roots of the eigenvalues of $Z'Z$ and equal to the trace of $(Z'Z)^\tfrac123$.

::: {#thm-procrustus}
$$
\max_{Y'Y=I}\text{tr}\ Y'XA=\|XA\|_\star
$$
:::
...
symmetric square root $\smash{(T'Y'YT)^\tfrac12}$
(or its negative, for a minimum). At the maximum $\text{tr}\ Y'XT=\text{tr}\ M=\|XT\|_\star$.
:::

Note that @thm-procrustus does not say that the optimizing $Y$ is unique. We get more insight from an alternative and more constructive proof (also taken from @deleeuw_C_04a, Appendix). It uses the "fat" singular value decomposition of the $n\times p$ matrix $XA$ of rank $m$.

::: {#thm-construct}
The $n\times m$ orthonormal $Y$ maximizes $\text{tr}\ Y'XA$, with 
\begin{equation}
XA=\begin{bmatrix}K&K_\perp\end{bmatrix}
...
\end{equation}
where the four-block partitioned matrix in the middle of \eqref{eq-odecomp} must be orthonormal again. Now $\text{tr}\ Y'XA=\text{tr}\ \Lambda P$ and the constraint on the $r\times r$ matrix $P$ is that $P'P\lesssim I$ in the Loewner sense (because we must have
$P'P+R'R=I$). It follows that the diagonal elements of $P$ must be less than or equal to one, and thus the maximum of
$\text{tr}\ Y'XA$ is $\text{tr}\ \Lambda$, uniquely attained for $P=I$.
...
choices of $S$ we have $\text{tr}\ Y'XA=\|XA\|_\star$, and thus the same loss 
function value.
...
\begin{equation}
T^{(k+1)}=\Pi_T\{CT^{(k)}(T^{(k)}CT^{(k)})^{-\tfrac12}\}
\end{equation}
:::
::: {.proof}
We rewrite \eqref{eq-yopt} as
$$
Y=XT(T'CT)^{-\tfrac12}+K_\perp^{\ } SL_\perp'
$$
where $(T'CT)^{-\tfrac12}$ is the square root of the Moore-Penrose inverse of 
$T'CT$. Thus
$$
...
$$
\mathcal{D}_s\sigma_\star=\text{tr}\ T'T_s-\text{tr}\ (T'CT)^{-\tfrac12}T'CT_s
$$
...
hasrecently beeen studied by @terada_25.
