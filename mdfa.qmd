...
\begin{equation}
\sigma(A,D)=\frac12\text{SSQ}(C-AA'-D^2).\label{eq-minrescfa}
\end{equation}
...
problem becomes a form of principal component analysis (PCA), in which the solution can be computed with a single sinu=gular value decomposition.
...
$$
T=\Pi_{\mathcal{T}}(X'Y)=\begin{bmatrix}A&\mid&D\end{bmatrix}=\begin{bmatrix}X'F&\mid&\text{diag}(X'U)\end{bmatrix}.
$$ 
...
\begin{equation}
T\leftarrow\Pi_{\mathcal{T}}(X'Y)=\Pi_{\mathcal{T}}\left\{C(T'CT)^{-\frac12}+X'K_\perp^{\ } SL_\perp'\right\}
\end{equation}
...
Algorithm A, or updating in CFA $F$ and $U$ separately, as in Algorithm C, we now propose Algorithm D, which
...
The optimal $y_j$ is proportional to $(I-Y_{-jY_{-j}'})z_j$. In Algorithm D we cycle once through
all columns before updating $T$ again. 
...
\begin{align}
\sigma_\star(T):=&\frac12\text{tr}\ X'X+\frac12\text{tr}\ T'T-\|XT\|_\star\notag\\=&\frac12\text{tr}\ X'X+\frac12\text{tr}\ T'T-\sum_{\nu=1}^m\lambda_\nu^\frac12(T'CT).\label{eq-projloss}
\end{align}
First and second derivatives of the trace norm with respect to $T$ were computed in @deleeuw_E_25d. The gradient is
\begin{equation}
\mathcal{D}\sigma_\star(T)=T-CT(T'CT)^{-1/2}.\label{eq-projgradsig}
\end{equation}
...
Since $\|XT\|_\star$ is convex, homogeneous, and under the constant rank assumption differentiable in $T$ 
the algorithm \eqref{eq-fixedp} is also a majorization (or MM) algorithm.
...
In Algorithms A and B the projection $\Pi_{\mathcal{T}}()$ can be quite general. Algorithm $G$ uses the binary template $B$ to code fixed and variable elements, and projection is just elementwise multiplication with the
...
$\theta_s$ then we can also incorporate inequality between the emlemnts of $T$, i.e. $\mathcal{T}$ can be a polyhedral convex cone.

For the parametrization \eqref{eq-sumpar}
\begin{equation}
\mathcal{D}_s\sigma_\star(T)=\text{tr}\ T'T_s-\text{tr}\ (T'CT)^{-1/2}T'CT_s.
\end{equation}
