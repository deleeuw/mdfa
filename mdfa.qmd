...
\end{equation}
where $\text{SSQ}()$ stands for the unweighted sum of squares. Minimizing \eeref{eq-minres)\} is the Unweighted Least Squares or ULS method of factor analysis.
...
$X'Y$ on the set of matrices $\mathcal{T}$. We shall use the symbol $\Pi_{\mathcal{T}}()$
...
role in Algorithm A, which uses $X'Y$ to update $T$, in the fitted values, which use $YT'$, 
and in the loss function values, which use $T'X'Y$.
...
T^{(k+1)}&=\Pi_{\mathcal{T}}(X'Y^{(k)}).\label{eq-als2}
\end{align}
\end{subequations}
...
advantages in secondary analysis. And factor analys publications have
a lot of secondary analysis, using the mountain of correlation matrices 
that have been published since 1900.

::: {#thm-als}
Suppose $T^{(k)}$ has rank $m$. Then
\begin{equation}
T^{(k+1)}=\Pi_{\mathcal{T}}\{CT^{(k)}(T^{(k)}CT^{(k)})^{-1/2}\}.\label{eq-alsshort}
\end{equation}
:::
::: {.proof}
...
\begin{equation}
T^{(k+1)}=\Pi_{\mathcal{T}}(X'Y)=\Pi_{\mathcal{T}}\left\{CT^{(k)}(\{T^{(k)}\}'CT^{(k)})^{-\frac12}+X'K_\perp^{\ } SL_\perp'\right\}
\end{equation}
...
\begin{equation}
A\leftarrow\Pi_{\mathcal{A}}(X'F)=\Pi_{\mathcal{A}}(X'U_\perp^{\ } U_\perp'XA
(A'X'U_\perp^{\ } U_\perp'XA)^{-1/2}).\label{eq-aupdate}
\end{equation}
Now use
$U_\perp^{\ } U_\perp'=I-UU'$ and define $\tilde D:=X'U$,
so that $D=\Pi_{\mathcal{D}}(\tilde D)$. Then \eqref{eq-aupdate} becomes
\begin{subequations}
\begin{equation}
...
$$
Suppose we have a current $F$ and $A=X'F$. Solve the MTFA problem of maximixizing $\text{tr}\ D^2$
...
We can get slightly more generality by defining $\mathcal{T}$ as the set of matrices of the form
\begin{equation}
T=T_0+\sum_{s=1}^h\theta_s T_s,\label{eq-sumpar}
\end{equation}
which allows, for example, for patterns with some elements fixed to known values and some elements required to be equal to other elements. Fixed elements go into $T_0$ and the $T_s$ with $s>0$
have zeroes where the fixed elements are. If the fixed
elements are all fixed to zero, as they are in exploratory
CFA, there is no need for a $T_0$.

For the parametrization \eqref{eq-sumpar}
\begin{equation}
\mathcal{D}_s\sigma_\star(T)=\text{tr}\ T'T_s-\text{tr}\ (T'CT)^{-1/2}T'CT_s.
\end{equation}
If $T_s$ is a unit matrix of the form $e_je_g'$, i.e. $T_s$ only has the single non-zero element $(j,g)$ equal to one, then  
\begin{equation}
\mathcal{D}_s\sigma_\star(T)=t_{jg}-\{(T'CT)^{-1/2}T'C\}_{jg}.
\end{equation}
