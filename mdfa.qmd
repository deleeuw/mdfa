...
*decomposition of order* $p$ of $X$. 
:::

Decompositions of various kinds play a key role in 
...
::: {#def-factanal}
A decomposition $XT'$ of order $p$ of the $n\times m$ matrix $X$ is a *factor analytic decomposition of order* $p$, if $m<p<n$ and $Y'Y=I$. 
:::

Note that $Y$ is a "tall" matrix ($n\geq p$), while $T$ is "wide" ($m\leq p$). The order $p$ of the decomposition is consequently larger than the rank of the matrix $X$. In order to have a non-trivial
data analysis technique we need constraints on $T$, which we write in the general form $T\in\mathcal{T}$, with $\mathcal{T}$ a subset of $\mathbb{R}^{m\times p}$, the space of all $m\times p$ matrices.
...
factor analytic decomposition, the set $\mathcal{T}$ is a set of
...
If $A$ is unrestricted and $D$ is required to be diagonal then the CFA is *exploratory*, otherwise it is *confirmatory*. If $D$ is diagonal then the squares of its diagonal elements are called 
...
"fat" SVD for X is the decomposition
\begin{equation}
X_{n\times m}=
\begin{bmatrix}
K&K_\perp
...
$$
\Pi_{\mathcal{Y}}(X):=\mathop{\text{argmin}}_{Y\in\mathcal{Y}}\ \text{SSQ}(X-Y)
$$
:::
...
\end{equation}
where $\text{SSQ}()$ stands for the unweighted sum of squares. Minimizing \eqref{eq-minres)} is the *Unweighted Least Squares (ULS)* method of factor analysis.
...
A more direct way of fitting the full CFA model was discussed (independently and around the same time) in the dissertation of @socan_03 and in the conference chapter of @deleeuw_C_04a (presented at the conference in 2002). Recently this technique has become known as *Matrix Decomposition Factor analysis (MDFA)*. Socan attributes MDFA (which they call *Direct Factor Analysis*) to a 2001 personal communication and some unpublished notes of their advisor Henk Kiers.
...
\begin{equation}
\begin{bmatrix}A&\mid&D\end{bmatrix}=\Pi_{\mathcal{T}}(X'Y)=\begin{bmatrix}X'F&\mid&\text{diag}(X'U)\end{bmatrix}.
\end{equation}
For confirmatory MDFA in which we require some loadings to be equal to each other and/or others to be equal to given constants finding $\Pi_{\mathcal{T}}(X'Y)$ is still a simple linear least squares problem. If there are inequality constraints we need some form of quadratic programming.
...
\begin{subequations}
\begin{align}
Y\leftarrow&\Pi_Y(XT),\label{eq-als1}\\
...
choices of $S$ we have $\text{tr}\ Y'XT=\|XT\|_\tau$, and all choices of $S$ give
...
Algorithm \eqref{eq-alsshort} requires us to compute $(T'CT)^{-\frac12}$ in each iteration, which means finding the $m$ non-zero eigenvalues and their corresponding eigenvectors. The update is then projected on the set $\mathcal{T}$. The ALS interpretation of the
...
this extends Algorithm B. On the other hand if $T$ has rank $m$ then
...
Write $\Pi_{\mathcal{A}}()$ and $\Pi_{\mathcal{D}}()$ for the least squares projectors on $\mathcal{A}$ and
$\mathcal{D}$. 

In Algorithm A we use a two-block ALS strategy. We alternate the update of $Y=(F\mid U)$ for given $T=(A\mid D)$ with the update of $T=(A\mid D)$ for given $Y=(F\mid U)$. In Algorithm $C$ we use
...
The stationary equations are $XA=FM+UN$, with $M$ and $N$ matrices of Lagrange multipliers, with
$M$ symmetric. Premultiplying by $U'$ gives $N=U'XA$ and thus $(I-UU')XA=FM$. Thus $F$ can be found from
the SVD of $(I-UU')XA$. It is convenient to define $\tilde D:=U'X$, so that $(I-UU')XA=(X-U\tilde D)A$.
In the same way we maximize $\text{tr}\ U'XD$ over $U$ satisfying $U'U=I$ and $U'F=0$. This
leads to generalized Procrustus for $XD=FM+UN$ $M=F'XD$ $XD-FF'XD=(I-FF')XD=(X-F\tilde A')D$.

Clearly Algorithm C produced a decreasing sequence of loss function values, bounded below by zero,
which consequently converges. Moreover solutions for $(F, A, U, D)$ are in a compact set, and the
updates are continuous functions. Thus solution sequences have subsequences converging to
stationary points. But there is a problem. 
