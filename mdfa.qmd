...
\begin{equation}
T^{(k+1)}=\Pi_T\{CT^{(k)}(T^{(k)}CT^{(k)})^{-\tfrac12}\}.\label{eq-alsshort}
\end{equation}
:::
::: {.proof}
...
$0\leq d_j^2\leq c_{jj}$

We have not shown that $D^2\lesssim C$

## Scale Free

$$
\text{tr}\ (X-FA'-UD)W(X-FA'-UD)'=\text{tr}\ XWX'+\text{tr}\ A'WA+\text{tr}\ DWD-2\ \text{tr}\ A'WX'F
-2\ \text{tr}\ DWX'U
$$
$$
