...
iterations. First and second derivatives are taken from @deleeuw_E_25d. Eigenvalue/eigenvectors calculations use the RSpectra package (@qiu_mei_24).
...
Also
\begin{equation}
\sigma(T,Y)=\text{tr}\ C+\text{tr}\ T'T-2\ \text{tr}\ Y'XT.\label{eq-sigmay}
\end{equation}
...
::: {#cor-unique}
If $\text{rank}(T)=m$ and $Y$ is given by \eqref{eq-yoptt} then $X'Y=X'KL'=CT(T'CT)^{-1/2}$. If $\text{rank}(X)=m$ then $YT'=KL'T'=XT(T'CT)^{-1/2}T'$.
:::
::: {.proof}
We know that $K_\perp'XT=0$. If $\text{rank}(T)=m$ this implies $X'K_\perp=0$.
If $\text{rank}(X)=m$ then $XTL_\perp=0$ implies $L_\perp'T=0$. In both cases
the indeterminate part of $Y$ contributes zero.
:::

The repository has a file mdfaAlgorithmA.R, with the function mdfaAlgorithmA() implementing
Algorithm A. There is a function mdfaProject() which take care of the least squares projections on $\mathcal{T}$. By default mdfaProject() replaces the matrix with the last $m$ columns of $T$ by its diagonal, but replacing the default projection routine allows us to incorporate any set of constraints. We emphasize that mdfaAlgorithmA() updates $Y$
using the dtertimate part $KL'$ only, which means that we do not have $Y'Y=I$. 
@cor-unique show that this does not impact the sequences of loss function values and of iterates for $T$, but it does mean that if we want a proper $Y$ then we have to do some extra work after convergence. @adachi_12, section 2.5, makes a case for using only the determinate
part, but it remains true that this does not solve the MDFA problem, which requires $Y'Y=I$.
...
but without the ugly superscripts. We have
\begin{subequations}
\begin{equation}
F\leftarrow U_\perp V=U_\perp^{\ } U_\perp'XA
(A'X'U_\perp^{\ } U_\perp'XA)^{-1/2}.
\end{equation}
In the same way the optimum $U$ for given $(F,A,D)$ is
\begin{equation}
U\leftarrow F_\perp^{\ } F_\perp'XD
(DX'F_\perp^{\ } F_\perp'XD)^{-1/2}.
\end{equation}
\end{subequations}
In each ALS cycle we update in the order $(F,A,U,D)$. Using $\leftarrow$ for the
update, we have
\begin{equation}
...
so that $D=\text{diag}(\tilde D)$. Then ... becomes
\begin{subequations}
\begin{equation}
A\leftarrow \Pi_A\left\{(C-\tilde D\tilde D')A[A'(C-\tilde D\tilde D')A]^{-1/2}\right\}
\end{equation}
...
$$
\mathcal{D}\sigma_\star(T)=T-CT(T'CT)^{-1/2}
$$
...
... (code in the appendix) uses the BFGS method of the optim() function from the stats package (@r_core_team_25). 


```{r}
hemm <- mdfaAlgorithmG(emmett, aemm, temm)
print(hemm)
hmax <- mdfaAlgorithmG(maxwell, memm, uemm)
print(hmax)
```
...
h <- mdfaAlgorithmB(emmett, aemm, temm, itmax = 1000, verbose = FALSE)
```
...
```{r alga_d, echo = FALSE}
matrixPrint(h$uniquenesses, digits = 3, width = 5, flag ="")
```
...
```{r alga_a, echo = FALSE}
matrixPrint(-h$loadings, digits = 3, width = 5)
```
...
```{r mdfa_plot, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 6}
par(pty="s")
mdfa <- tcrossprod(h$loadings) + diag(h$uniquenesses)
plot(emmett, mdfa, col = "RED")
abline(0, 1)
```

```{r ml_plot, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 6}
par(pty="s")
ml <- tcrossprod(ml_a) + diag(mld)
plot(emmett, ml, col = "RED")
abline(0, 1)
```
The Emmett example is somewhat atypical, because the CFA model with three factors has a near perfect fit. This explains why the
MDFA and ML solutions are nearly identical.

```{r}
hemm <- mdfaAlgorithmG(emmett, aemm, temm)
```

The same solution is reached using Algorithm G.
It requires 29 function evaluations and 27 gradient evaluations to arrive 
at `r hemm$loss`.
...
h <- mdfaAlgorithmB(maxwell, memm, uemm, itmax = 10000, verbose = FALSE)
```
...
```{r maxmdfa_plot, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 6}
par(pty="s")
mdfa <- tcrossprod(h$loadings) + diag(h$uniquenesses)
plot(maxwell, mdfa, col = "RED")
abline(0, 1)
```

```{r maxml_plot, echo = FALSE, fig.align = "center", fig.width = 6, fig.height = 6}
par(pty="s")
ml <- tcrossprod(maxml_a) + diag(maxmld)
plot(maxwell, ml, col = "RED")
abline(0, 1)
```
...
The CFA definition ...-... implies $A=X'F$ and 
...
diagonal then this indicates a lack of CFA fit. 
It may also be sufficiently counter-intuitive to prevent 
it from happening.

In the thesis of Socan ab ALS variation of MDFA is discussed in which it is required, in addition to $Y'Y=I$, that $U'X$ is diagonal. In the version of MDFA in @stegeman_16, which they call the *Data Factor Model*,
...
For many CFA techniques it is possible that the estimated
uniqueness do not satisfy these inequality constraints
and are consequently "improper" or Heywood cases.

::: {#thm-bounds}
In the MDFA solution $0\lesssim AA'\lesssim C$ and 
...
$d_j=x_j'u_j$. Since $u_j'u)j=1$  by Cauchy-Schwartz 
$d_j^2\leq x_j'x_j=c_{jj}$.
:::

@thm-bounds shows that worst-case Heywood cases 
