...
$X'Y$ on the set of matrices $\mathcal{T}$. We shall use the symbol $\Pi_T()$
for the unweighted least squares projection on $\mathcal{T}$.

For exploratory CFA this gives 
$$
T=\Pi_T(X'Y)=\begin{bmatrix}A&\mid&D\end{bmatrix}=\begin{bmatrix}X'F&\mid&\text{diag}(X'U)\end{bmatrix}.
$$ 
For confirmatory MDFA in which we require some loadings to be equal to each other and/or others to be equal to given constants finding $\Pi_T(X'Y)$ is still a simple linear least squares problem. If there are inequality constraints we need some form of quadratic programming.
...
\begin{equation}
\mathcal{D}\|X\cdot\|_\star(T)=CT(T'CT)^{-1/2}.\label{eq-projgradnrm}
\end{equation}
from which wer see that
\begin{equation}
\mathcal{D}\sigma_\star(T)=T-CT(T'CT)^{-1/2}.\label{eq-projgradsig}
\end{equation}
Thus the iterative algorithm
$$
T\leftarrow CT(T'CT)^{-1/2}
$$
is a fixed-point algorithm, and $T\in\mathcal{T}$ is a fixed point of ... if and only if $\mathcal{D}\sigma_\star(T)=0$.
Since $\|XT\|_\star(T)$ is convex, homogeneous, and (under our assumptions) differentiable in $T$ 
algorithm ... is also a majorization (or MM) algorithm.

If the projection $\Pi_T()$ is such that it replaces a some elements of $T$ by zeroes, then we can use
the projected gradient
\begin{equation}
\mathcal{D}\sigma_\star(T)=\Pi_T(T-CT(T'CT)^{-1/2}).\label{eq-projgradcor}
\end{equation}
With this formula for the (projected) gradient many general purpose optimization methods for minimizing the projected loss function \eqref{eq-projloss} become available. The R function
mdfaAlgorithmG() in the repository uses the BFGS method of the optim() function from the stats package (@r_core_team_25). 

We can get slightly more generality by defining $\mathcal{T}$ as the set of matricves of the form
$$
T=T_0+\sum_{s=1}^q\theta_s T_s,
$$
which allows, for example, for patterns with some elements fixed to known values and some elements required to be equal to other elements.
$$
...
the general projections $\Pi_T()$ available.
...
@maxwell_61 (ten variables, 810 observations). The correlation matrix
is
...
analyzed with ML by @joreskog_69.

## Cattell

This example uses the correlation matrix between 12 cognitive variables from @cattell_63,
included in the psych package (@revelle_25), to illustrate fitting a group factor model.
The variables 1 and 2 measures verbal ability, 3 and 4 are spatial ability, 5 and 6 are reasoning, 7 and 8 are numerical ability. The first eight are from the Thurstone Primary Abilities test, and the last four are from the (IPAT) Culture Fair Intelligence Test.

We apply Algorithm B with 6 factors: one general factor and five group factors. The projection $\Pi_A()$ copies all
elements from the first column of $X'F$, elements one and two from column two, three and four from column three, five and six from column four, seven and eight from column five, and nine to twelve from column six. All other elements of $\Pi_A(X'F)$ are zero, and $\Pi_D(X'F)=\text{diag}(X'F)$ as usual.
...
```{r}
r <- diag(6)
r[-1, -1] <- matrix(rnorm(25), 5, 5)
matrixPrint(r)
matrixPrint(tcrossprod(r))
h <- g$loadings %*% r
matrixPrint(h[1, -1] / h[2, -1])
matrixPrint(h[3, -1] / h[4, -1])
print(svd(h[9:12, -1]))
```

@rindskopf_rose_88 also do a group factor analysis, but they allow for correlations
between the five group factors. This amounts to imposing the structure
$$
A=\begin{bmatrix}
a_{11}&a_2&0&0&0&0\\
a_{21}&0&a_3&0&0&0\\
...
This shows that $\Pi_A()$ copies the first column of $X'F$ and computes five rank one approximations to the
...
h <- mdfaAlgorithmB(cattell, catsvd, cattellSVDProjection, itmax = 1000, verbose = FALSE)
```
